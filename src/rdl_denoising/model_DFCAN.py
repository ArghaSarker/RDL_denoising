...
def fft2(input):
    ...
    absfft = tf.pow(
        tf.abs(fft) + 1e-8, 0.1
    )  # gama correction: why? : Gamma operation on the power spectrums to enhance the contributions of high-frequency components.
...
def fftshift(input):
    ...
    output = tf.image.resize(
        output, (128, 128)
    )  # resize to 128x128 to make sure sampling consistancy.
    return output
